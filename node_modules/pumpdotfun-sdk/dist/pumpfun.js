...
class PumpFunSDK {
    ...
    async createAndBuy(creator, mint, createTokenMetadata, buyAmountSol, slippageBasisPoints = 500n, priorityFees, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        ...
        if (buyAmountSol > 0) {
            ...
            console.log(`buying $${createTokenMetadata.symbol}: ${Number(buyAmount) / 10 ** exports.DEFAULT_DECIMALS} for: ${Number(buyAmountWithSlippage) / Number(web3_js_1.LAMPORTS_PER_SOL)} SOL`);
            let buyTx = await this.getBuyInstructions(creator.publicKey, mint.publicKey, globalAccount.feeRecipient, buyAmount, buyAmountWithSlippage);
            newTx.add(buyTx);
        }
    ...
    async buy(globalAccount,bondingCurveAccount,buyer, mint, buyAmountSol, slippageBasisPoints = 500n, priorityFees, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
       // let bondingCurveAccount = await this.getBondingCurveAccount(mint, commitment);
        ...
        console.log(`buying ${Number(buyAmount) / 10 ** exports.DEFAULT_DECIMALS} for: ${Number(buyAmountWithSlippage) / web3_js_1.LAMPORTS_PER_SOL} SOL (${Number(buyAmountWithSlippage)})`);

        let buyTx:Transaction = await this.getBuyInstructions(buyer.publicKey, mint, globalAccount.feeRecipient, buyAmount, buyAmountWithSlippage).then(buyTx=>{
            console.log('transaction ',buyTx,Date.now())
    ...
    async sell(seller, mint, sellAmount, slippageBasisPoints = 500n, priorityFees, commitment = util_1.DEFAULT_COMMITMENT, finality = util_1.DEFAULT_FINALITY) {
        ...
        let minSolOutput = bondingCurveAccount.getSellPrice(sellAmount, globalAccount.feeBasisPoints);
        let sellAmountWithSlippage = (0, util_1.calculateWithSlippageSell)(minSolOutput, slippageBasisPoints);
        console.log(`selling ${Number(sellAmount) / 10 ** exports.DEFAULT_DECIMALS} for: ${Number(sellAmountWithSlippage) / web3_js_1.LAMPORTS_PER_SOL} SOL`);
        let sellTx = await this.getSellInstructions(seller.publicKey, mint, globalAccount.feeRecipient, sellAmount, sellAmountWithSlippage);
    ...
    async getBondingCurveAccount(mint, commitment = util_1.DEFAULT_COMMITMENT) {
        let PDA =this.getBondingCurvePDA(mint);
        console.log('pda ',PDA.toBase58())
        const tokenAccount = await this.connection.getAccountInfo(PDA, commitment);
    ...
    async createTokenMetadata(create) {
        let file = await (0, node_fs_1.openAsBlob)(create.filePath);
